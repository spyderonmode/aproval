...
- **Mobile Leaderboard Complete Fix** (July 19, 2025): Fixed critical mobile responsiveness issues with leaderboard modal for Arabic and all language interfaces. Moved leaderboard button from sidebar to main header for better mobile accessibility. Optimized modal sizing from fixed max-w-5xl to responsive max-w-[95vw] on mobile devices. Made performance badges ("Expert", "Improving", "Elite") visible on mobile by removing hidden lg:flex restrictions. Added responsive W-L-D stats display for all screen sizes. Reduced profile picture sizes, spacing, and text dimensions for mobile optimization. Leaderboard now displays complete information including performance badges and win-loss-draw statistics on all devices, particularly Arabic mobile interface.
