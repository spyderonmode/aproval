...
- **Achievement Synchronization Fix** (July 18, 2025): Fixed critical bug where achievements were incorrectly granted based on >= conditions instead of exact milestone achievements. Users were receiving "Champion" achievement (100 wins) when they only had 51 wins. Root cause was achievement logic using >= comparisons for milestone achievements, granting all lower achievements simultaneously. Fixed by updating checkAndGrantAchievements() to use exact milestone conditions (e.g., wins === 50 for Legend, wins === 100 for Champion). Added recalculateUserAchievements() function to clean up existing incorrect achievements and recalculate based on current stats. Added "Fix Achievements" button in Achievement Modal for users to sync their achievements with actual stats. Achievement system now properly grants achievements only when exact milestones are reached.
