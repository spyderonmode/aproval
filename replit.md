...
- **Invitation Polling Pause System** (July 18, 2025): Implemented smart invitation polling system that automatically stops API calls after users accept or reject invitations. Added "Check Invitations" button in header menu allowing users to manually resume invitation polling when needed. System prevents unnecessary continuous API polling by pausing invitation checks after user interactions until manually triggered again. Enhanced authentication checks in all polling components to completely stop API calls for unauthenticated users.
