...
- **Unread Message Indicators & Inbox Prioritization** (July 15, 2025): Added comprehensive unread message system with green dot indicators on profile pictures, chat buttons, and user cards. Users with unread messages automatically move to the top of the online players list with green highlighting. Messages are marked as read when user opens chat, and all indicators are cleared when users go offline
