...
- **Dedicated Arabic User List for Mobile** (July 19, 2025): Created completely separate Arabic-only user ranking system for mobile users to solve persistent layout issues. Features include: new `/api/arabic-rankings` endpoint returning simplified user data (max 50 users), dedicated `ArabicUserList` component with native RTL support and Arabic fonts, automatic mobile detection to show Arabic list for Arabic users on mobile devices, clean card-based layout with rank indicators, profile pictures, and stats display, proper Arabic text with LTR numbers using span dir="ltr", simplified header "أفضل اللاعبين" (Best Players) without complex animations, and mobile-optimized spacing and typography. Arabic mobile users now get a dedicated, clean interface that avoids all the layout conflicts of the complex leaderboard design.
