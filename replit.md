...
- **Leaderboard Arabic Text Removal & Layout Fix** (July 19, 2025): Removed all Arabic text from the leaderboard area while maintaining Arabic language support for other parts of the application. Changed leaderboard content to display in English for all languages including title "Leaderboard", subtitle "Top 100 Players", and all player statistics (Win Rate, games, W-L-D, Wins, Losses, Draws). Added dir="ltr" attribute to force left-to-right layout for leaderboard modal even when Arabic language is selected, ensuring proper English text alignment and layout consistency.
