...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get user's coins
  app.get('/api/shop/coins', requireAuth, async (req: any, res) => {
    try {
      const userId = req.session.user.userId;
  ...
  app.get('/api/shop/purchases', requireAuth, async (req: any, res) => {
    try {
      const userId = req.session.user.userId;
  ...
  app.get('/api/shop/inventory', requireAuth, async (req: any, res) => {
    try {
      const userId = req.session.user.userId;
  ...
  app.post('/api/shop/coins/add', requireAuth, async (req: any, res) => {
    try {
      ...
      await storage.addCoins(userId, amount);
      const newBalance = await storage.getUserCoins(userId);
